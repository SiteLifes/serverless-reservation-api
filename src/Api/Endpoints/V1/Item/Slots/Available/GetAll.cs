...
public class GetAll : IEndpoint
{
    ...
    )
    {
        ...
        var reservations =
            await reservationService.GetReservationsAsync(id, now, maxDate, cancellationToken);
        ...
        {
            var workingHours =
                config.WorkingHours.FirstOrDefault(q => q.DayOfWeek == currentDate.DayOfWeek);
            ...
            while (workingHours.Open <= workingHours.Close)
            {
                ...
                slotHours.Add(new SlotDto.SlotHourDto
                {
                    ...
                    IsAvailable = (config.SlotCountAtSameTime > reservationCountInThatSlot)
                                  && currentDate.Add(workingHours.Open) > now,
